...
public final class OperationParser {
	...
	private AbstractOperation parseVcsOperation(String newOperation,
		...
		//System.out.println(operationArgs);
		...
		default:
			return operationFactory.createOperation(
					OperationType.VCS_INVALID_OPERATION,
					operationArgs);
		}
	}
	...
	private AbstractOperation parseFileSystemOperation(String newOperation,
		...
		case "cd":
			return operationFactory.createOperation(
					OperationType.CHANGEDIR, operationArgs);
		case "ls":
		...
		case "rmdir":
			...
			return operationFactory.createOperation(
					OperationType.REMOVE, operationArgs);
		case "touch":
			if (operationArgs.size() == 1) {
				parsedArgs = parsePathAndName(operationArgs.get(0));
			}
		...
		case "writetofile":
			...
			return operationFactory.createOperation(
					OperationType.WRITETOFILE, parsedArgs);
		case "print":
			return operationFactory.createOperation(OperationType.PRINT, operationArgs);
		default:
			return operationFactory.createOperation(
					OperationType.FILESYSTEM_INVALID_OPERATION,
					operationArgs);
		}
	}
